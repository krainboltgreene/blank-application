defmodule ExampleWeb.Graphql.Resolvers.Accounts do
  @spec list(any, any, any) :: {:ok, [%Example.Models.Account{}]} | {:error, any}
  def list(_parent, _arguments, _resolution) do
    {:ok, Example.Database.Repo.all(Example.Models.Account)}
  end

  @spec find(any, %{input: %{id: bitstring}}, any) ::
          {:ok, %Example.Models.Account{}} | {:error, any}
  def find(_parent, %{input: %{id: id}}, _resolution) when not is_nil(id) and is_bitstring(id) do
    {:ok, Example.Database.Repo.get(Example.Models.Account, id)}
  end
  ...
  def create(_parent, %{input: %{email: email} = input}, _resolution) do
    ...
    %Example.Models.Account{}
    |> Example.Models.Account.changeset(attributes)
    |> case do
  ...
  def update(_parent, %{input: %{id: id} = input}, _resolution) when is_bitstring(id) do
    ...
    |> Example.Models.Account.changeset(input)
    |> case do
      %Ecto.Changeset{valid?: true} = changeset -> Example.Database.Repo.insert(changeset)
      %Ecto.Changeset{valid?: false} = changeset -> {:error, changeset}
    end
  ...
  def grant_administration_powers(_parent, %{input: %{id: id}}, _resolution)
    ...
    Example.Database.Repo.get!(Example.Models.Account, id)
    |> Example.Models.Account.grant_administrator_powers!()
  end
  ...
  def revoke_administration_powers(_parent, %{input: %{id: id}}, _resolution)
      when is_bitstring(id) do
    Example.Database.Repo.get!(Example.Models.Account, id)
    |> Example.Models.Account.revoke_administrator_powers!()
  end
end
