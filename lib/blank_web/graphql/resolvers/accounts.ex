defmodule BlankWeb.Graphql.Resolvers.Accounts do
  @spec list(any, any, any) :: {:ok, [%Blank.Models.Account{}]} | {:error, any}
  def list(_parent, _arguments, _resolution) do
    {:ok, Blank.Database.Repo.all(Blank.Models.Account)}
  end

  @spec find(any, %{input: %{id: bitstring}}, any) ::
          {:ok, %Blank.Models.Account{}} | {:error, any}
  def find(_parent, %{input: %{id: id}}, _resolution) when not is_nil(id) and is_bitstring(id) do
    {:ok, Blank.Database.Repo.get(Blank.Models.Account, id)}
  end
  ...
  def create(_parent, %{input: %{email: email} = input}, _resolution) do
    ...
    %Blank.Models.Account{}
    |> Blank.Models.Account.changeset(attributes)
    |> case do
  ...
  def update(_parent, %{input: %{id: id} = input}, _resolution) when is_bitstring(id) do
    Blank.Database.Repo.get!(Blank.Models.Account, id)
    |> Blank.Models.Account.changeset(input)
    |> case do
      %Ecto.Changeset{valid?: true} = changeset -> Blank.Database.Repo.insert(changeset)
      %Ecto.Changeset{valid?: false} = changeset -> {:error, changeset}
    end
  ...
  def grant_administration_powers(_parent, %{input: %{id: id}}, _resolution)
    ...
    Blank.Database.Repo.get!(Blank.Models.Account, id)
    |> Blank.Models.Account.grant_administrator_powers!()
  end
  ...
  def revoke_administration_powers(_parent, %{input: %{id: id}}, _resolution)
      when is_bitstring(id) do
    Blank.Database.Repo.get!(Blank.Models.Account, id)
    |> Blank.Models.Account.revoke_administrator_powers!()
  end
end
