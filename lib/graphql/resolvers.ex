defmodule Graphql.Resolvers do
  ...
  defmacro creatable(model, :authenticated) do
    quote do
      @spec create(any, %{input: map}, %{context: %{:current_account => %Database.Models.Account{id: String.t()}}}) :: {:ok, %unquote(model){}} | {:error, %Ecto.ChangeError{} | :internal_server_error}
      def create(_parent, %{input: input}, %{context: %{current_account: current_account}}) when not is_nil(current_account) do
        %unquote(model){}
  ...
  defmacro updatable(model, :authenticated) do
    quote do
      ...
      def update(_parent, %{input: %{id: id} = input}, %{context: %{current_account: current_account}}) when is_bitstring(id) do
  ...
  defmacro destroyable(model, :authenticated) do
    quote do
      @spec destroy(any, %{input: %{id: String.t()}}, %{context: %{:current_account => %Database.Models.Account{id: String.t()}}}) ::
              {:ok, :no_content} | {:error, %Ecto.ChangeError{} | :internal_server_error}
      def destroy(_parent, %{input: %{id: id}}, %{context: %{current_account: current_account}}) when is_bitstring(id) do
        Database.Repository.get!(unquote(model), id)
        |> Database.Repository.delete()
      end
      @spec destroy(any, any, %{context: %{:current_account => nil}}) :: {:error, :unauthenticated}
  ...
  defmacro listable(model, :authenticated) do
    quote do
      require Ecto.Query
      @default_limit 10
      @spec list(any, %{optional(:input) => %{limit: integer}}, %{context: %{current_account: %Database.Models.Account{id: String.t()}}}) :: {:ok, list(%unquote(model){})}
      def list(_parent, %{input: %{limit: limit}}, %{context: %{current_account: current_account}}) when is_integer(limit) and limit > 0 and not is_nil(current_account) do
  ...
  defmacro findable(model, :authenticated) do
    quote do
      @spec find(any, %{input: %{id: String.t()}}, %{context: %{current_account: %Database.Models.Account{id: String.t()}}}) :: {:ok, %unquote(model){} | nil}
