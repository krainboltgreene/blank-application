...
export default function Field (properties) {
  ...
  if (type === "checkbox" || type === "radio") {
    return <section className="form-group form-check">
      <Input id={inputId} value={value} onChange={onChange} attributes={inputAttributes} />
      <Label htmlFor={inputId} attributes={labelAttributes}>{label}</Label>
  ...
  return <section className="form-group">
    <Label htmlFor={inputId} attributes={labelAttributes}>{label}</Label>
    <Input id={inputId} type={type} value={value} onChange={onChange} attributes={inputAttributes} />
    <Help id={inputId}>{help}</Help>
    <Feedback isValid={isValid}>{feedback}</Feedback>
  </section>;
}
