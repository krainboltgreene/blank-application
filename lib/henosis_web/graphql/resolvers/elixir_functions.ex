defmodule HenosisWeb.Graphql.Resolvers.ElixirFunctions do
  ...
  def list(_parent, _arguments, _resolution) do
    {:ok, Henosis.Database.Repo.all(Henosis.Models.ElixirFunction)}
  end
  ...
  def find(_parent, %{input: %{id: id}}, _resolution) when not is_nil(id) and is_bitstring(id) do
    {:ok, Henosis.Database.Repo.get(Henosis.Models.ElixirFunction, id)}
  end
  ...
  def update(_parent, %{input: %{id: id} = input}, _resolution) when is_bitstring(id) do
    Henosis.Database.Repo.get!(Henosis.Models.ElixirFunction, id)
    |> Henosis.Models.ElixirFunction.changeset(input)
    |> case do
      %Ecto.Changeset{valid?: true} = changeset -> Henosis.Database.Repo.insert(changeset)
      %Ecto.Changeset{valid?: false} = changeset -> {:error, changeset}
