defmodule <%= inspect schema.repo %>.Migrations.Create<%= Macro.camelize(schema.table) %> do
  ...
  def change do
    create table(:<%= schema.table %><%= if schema.prefix do %>, prefix: :<%= schema.prefix %><% end %>) do
<%= for {k, v} <- schema.attrs do %>      add <%= inspect k %>, <%= inspect Mix.Phoenix.Schema.type_for_migration(v) %><%= schema.migration_defaults[k] %>
