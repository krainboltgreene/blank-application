defmodule <%= inspect context.web_module %>.<%= inspect Module.concat(schema.web_namespace, schema.alias) %>Controller do
  ...
  def new(conn, _params) do
    changeset = <%= inspect context.module %>.change_<%= schema.singular %>(%<%= inspect schema.module %>{})
  ...
  def update(conn, %{"id" => id, <%= inspect schema.singular %> => <%= schema.singular %>_params}) do
    ...
    case <%= inspect context.module %>.update_<%= schema.singular %>(<%= schema.singular %>, <%= schema.singular %>_params) do
      {:ok, <%= schema.singular %>} ->
        conn
  ...
  def delete(conn, %{"id" => id}) do
    <%= schema.singular %> = <%= inspect context.module %>.get_<%= schema.singular %>!(id)
    {:ok, _<%= schema.singular %>} = <%= inspect context.module %>.delete_<%= schema.singular %>(<%= schema.singular %>)
