...
if ! dpkg -s apt-transport-https curl ca-certificates > /dev/null 2>&1 || ! type gpg > /dev/null 2>&1; then
    apt-get-update-if-needed
    apt-get -y -qq install --no-install-recommends apt-transport-https curl ca-certificates gnupg2
fi
...
if type docker > /dev/null 2>&1; then
    ...
    if [ "${USE_MOBY}" = "true" ]; then
        ...
        apt-get -qq update
        apt-get -y -qq install --no-install-recommends moby-cli moby-buildx
    else
        # Import key safely (new 'signed-by' method rather than deprecated apt-key approach) and install
        curl -fsSL https://download.docker.com/linux/${ID}/gpg | gpg --dearmor > /usr/share/keyrings/docker-archive-keyring.gpg
        echo "deb [arch=$(dpkg --print-architecture) signed-by=/usr/share/keyrings/docker-archive-keyring.gpg] https://download.docker.com/linux/${ID} ${VERSION_CODENAME} stable" > /etc/apt/sources.list.d/docker.list
        apt-get -qq update
        apt-get -y -qq install --no-install-recommends docker-ce-cli
    fi
fi
...
if type docker-compose > /dev/null 2>&1; then
    ...
    if [ "${TARGET_COMPOSE_ARCH}" != "x86_64" ]; then
        ...
        if ! dpkg -s python3-minimal python3-pip libffi-dev python3-venv pipx > /dev/null 2>&1; then
            apt-get-update-if-needed
            apt-get -y -qq install python3-minimal python3-pip libffi-dev python3-venv pipx
        fi
